...
<div class="container-fluid mainBox">
    <div class="row ">
        ...
        <div class="col-sm-10 mainBox" style="padding:10px;margin: 0px">
            <div class="row" id="findUserMainBox">
                <div class="col-sm-12 clearfix">
                    <div class="col-sm-12 column">
                        <div class="page-header col-sm-12">
                            <h1>
                                查询用户 <small v-if="users.length == 0 ? false : true">共找到：<strong class="text-danger"> {{lineCount}} </strong>位用户</small>
                            </h1>
                        </div>
                    </div>
                </div>
                <div class="col-sm-12" style="margin-bottom: 15px">
                    <div class="row">
                        <div class="col-sm-offset-2 col-sm-4">
                            <input type="text" class="form-control " v-model="selInfo" @keyup.enter="findUserByPage"
                                   placeholder="请输入关键字"/>
                        </div>
                        <div class="col-sm-2">
                            <select class="form-control" id="selectTitle" @change="setTitle">
                                <option v-for="(item,index) in userTitle" :value="item.title">{{item.titleName}}
                                </option>
                            </select>
                        </div>
                        <button type="button" class="btn btn-primary col-sm-1" @click="findUserByPage">查询</button>
                    </div>
                </div>
                <div class="col-sm-12 text-center text-info" style="margin-top: 40px"
                     v-if="users.length == 0 ? true : false">
                    <h3>{{findTip}}</h3>
                </div>
                    ...
                    <div class="row userInfoBox">
                        ...
                        <div class="col-sm-3 text-left" style="padding-top: 15px">
                            ...
                            <div class="col-sm-9" style="padding:0px;margin-top: 5px">
                                <select class="form-control" @change="changeRole(item.role,item.id)">
                ...
                <div class="col-sm-12 text-center" v-if="users.length == 0 ? false : true">
                    <ul class="pagination">
                        ...
                        <li><a href="javascript:void(0)" v-for="index in pageCount">
                            ...
                            <span v-if="index == pageNow" class="text-danger"
                                  @click="findUsers(index)">{{ index }}</span>
                        </a></li>
                            ...
                            <div class="modal-body">
                                <form class="form-horizontal" role="form">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-6">
                                            <input type="text" class="form-control" id="userName" name="userName"
                                                   v-model="user.userName" @click="userNameOnclick"
                                                   @blur="findUserByName"
                                                   placeholder="请输入用户名">
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <label for="address" class="col-sm-3 control-label"><p
...
<script type="text/javascript">
    ...
    function findAllUserTitle() {
        var that = vue.$data
        axios.post('userTitle/findAllUserTitle', {})
            .then(function (response) {
                console.log(response)
                that.userTitle = response.data.userTitles;
            })
    }
    ...
    var vue = new Vue({
        ...
        methods: {
            setTitle: function () {
                this.selUserTitle = $("#selectTitle").val();
            },
            ...
            findUserByPage: function () {
                if (this.selInfo == null || this.selInfo.trim() == "") {
                    alert("请输入要查询的内容");
                    return;
                }
                var user = {
                ...
                if (this.selUserTitle == "id") {
                    user.id = this.selInfo;
                } else if (this.selUserTitle == "userName") {
                    user.userName = this.selInfo;
                } else if (this.selUserTitle == "password") {
                    user.password = this.selInfo;
                } else if (this.selUserTitle == "tel") {
                    user.tel = this.selInfo;
                } else if (this.selUserTitle == "address") {
                    user.address = this.selInfo;
                }
                var data = {
                ...
                var that = this;
                axios.post('user/findUsersByPageLimit', data)
                    .then(function (response) {
                        console.log(response)
                        that.users = response.data.users;
                        if (that.users.length < 1) {
                            that.findTip = "暂未查询到结果！";
                        }
