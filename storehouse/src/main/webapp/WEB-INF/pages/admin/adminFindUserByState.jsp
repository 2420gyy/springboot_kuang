            ...
            <div class="row clearfix">
                <div class="col-md-12 column">
                    <div class="page-header">
                        <h1>
                            未激活用户 <small v-if="users.length == 0 ? false : true">共找到：<strong class="text-danger"> {{lineCount}} </strong>位用户</small>
                        </h1>
                    </div>
                </div>
                <div class="col-sm-6 userBox" v-for="(item,index) in users">
                    <div class="row userInfoBox">
                        ...
                        <div class="col-sm-3 text-right" style="padding-top: 15px;padding-right: 30px">
                            <a href="javascript:void(0)" @click="removerUserById(item.id)" class="tooltip-test" data-toggle="tooltip"
                               title="删除用户">
...
<script type="text/javascript">
    $(function () { $("[data-toggle='tooltip']").tooltip(); }); //提示工具
    $(function () {
        findAllUsers();
    })
    ...
    var vue = new Vue({
        ...
        methods: {
            removerUserById:function(id){
                if(!confirm("删除后无法恢复，确定删除？")){
                    return;
                }
                axios.post('user/deleteUserById', {id: id})
                    .then(function (response) {
                        console.log(response)
